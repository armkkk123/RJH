...
function Library.new(config)
	...
	MegaGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
	...
	WarmGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
	...
	task.spawn(function()
		...
		-- ═══ PHASE 3: ENERGY CORE IGNITION (0.7s) ═══
		-- Glow explosion from center
		Twen:Create(MegaGlow, TI(0.8, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Size = UDim2.new(0, 350, 0, 350),
			ImageTransparency = 0.4
		}):Play()
		Twen:Create(WarmGlow, TI(1.0, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Size = UDim2.new(0, 250, 0, 250),
			ImageTransparency = 0.5
		}):Play()

		-- Rings burst open
		Twen:Create(OuterRing, TI(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			Size = UDim2.new(0, 130, 0, 130),
			ImageTransparency = 0.15
		}):Play()
		task.wait(0.1)
		Twen:Create(InnerRing, TI(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
		...
		task.spawn(function()
			while not LoadingDone do
				OuterRing.Rotation = OuterRing.Rotation + 5
				InnerRing.Rotation = InnerRing.Rotation - 3
				task.wait()
			end
		end)
		...
		task.spawn(function()
			while not LoadingDone do
				Twen:Create(MegaGlow, TI(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					ImageTransparency = 0.3, Size = UDim2.new(0, 420, 0, 420)
				}):Play()
				Twen:Create(WarmGlow, TI(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					ImageTransparency = 0.45, Size = UDim2.new(0, 300, 0, 300)
				}):Play()
				task.wait(1.0)
				Twen:Create(MegaGlow, TI(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					ImageTransparency = 0.55, Size = UDim2.new(0, 300, 0, 300)
				}):Play()
				Twen:Create(WarmGlow, TI(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
					ImageTransparency = 0.65, Size = UDim2.new(0, 200, 0, 200)
				}):Play()
				task.wait(1.0)
			end
		end)
		...
		-- ═══ PHASE 4: ENERGY CHARGE — Rings accelerate + glitch bursts (1.2s) ═══
		-- Ring pulse bigger
		Twen:Create(OuterRing, TI(0.5, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 160, 0, 160), ImageTransparency = 0.1
		}):Play()
		Twen:Create(InnerRing, TI(0.5, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 110, 0, 110), ImageTransparency = 0.3
		}):Play()

		-- Rapid glitch bursts
		for _ = 1, 5 do
			task.spawn(function()
				GlitchBar.BackgroundTransparency = math.random() * 0.3 + 0.1
				GlitchBar.Position = UDim2.new(0, 0, math.random() * 0.7 + 0.15, 0)
				GlitchBar.Size = UDim2.new(1, 0, 0, math.random(1, 3))
				task.wait(0.03)
				GlitchBar.BackgroundTransparency = 1
			end)
			task.wait(0.15)
		...
		-- Quick white micro-flash
		Twen:Create(WhiteFlash, TI(0.04), {BackgroundTransparency = 0.7}):Play()
		task.wait(0.05)
		Twen:Create(WhiteFlash, TI(0.15, Enum.EasingStyle.Quint), {BackgroundTransparency = 1}):Play()
		task.wait(0.2)

		-- ═══ PHASE 5: SUPERNOVA EXIT (~3.5s total) ═══
		LoadingDone = true
		...
		Twen:Create(InnerRing, TI(0.5, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 500, 0, 500), ImageTransparency = 1
		}):Play()

		-- Glow supernova
		Twen:Create(MegaGlow, TI(0.4, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 900, 0, 900), ImageTransparency = 0.1
		}):Play()
		Twen:Create(WarmGlow, TI(0.4, Enum.EasingStyle.Quint), {
			Size = UDim2.new(0, 700, 0, 700), ImageTransparency = 0.2
		}):Play()
		...
		-- Show MainFrame regardless
		if MainFrame then
			MainFrame.Visible = true
			Twen:Create(MainFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{BackgroundTransparency = 0.4}):Play()
		end
	end)
	-- ============================================================
	...
	MainFrame.Visible = false -- Hidden until loading finishes
