...
function Library.new(config)
	...
	local defaultSize = UDim2.new(0, 650, 0, 450)

	if isMobile and camera then
		-- หากเป็นมือถือให้ปรับขนาดตามหน้าจอกว้างคูณ 85%
		defaultSize = UDim2.new(0, camera.ViewportSize.X * 0.85, 0, camera.ViewportSize.Y * 0.85)
	end
	...
	local function Update()
		if WindowTable.WindowToggle then
			Twen:Create(MainFrame,TweenInfo.new(1.5,Enum.EasingStyle.Quint),{Position = UDim2.new(0.5, 0, 0.5, 0), BackgroundTransparency = 0.4,Size = config.Size}):Play();
			...
			Twen:Create(BlockFrame2,TweenInfo1,{BackgroundTransparency = 0.5}):Play();
			Twen:Create(BlockFrame3,TweenInfo1,{BackgroundTransparency = 1}):Play();

			Twen:Create(TabButtonFrame,TweenInfo1,{Position = UDim2.new(0.33, 0, 0, 65)}):Play();
			Twen:Create(MainTabFrame,TweenInfo1,{Position = UDim2.fromScale(0.5,0.27)}):Play();
			Twen:Create(Description,TweenInfo1,{Position = UDim2.new(0.33, 0, 0, 34)}):Play();

			Twen:Create(Title,TweenInfo1,{Position = UDim2.new(0.33, 0, 0, 12)}):Play();
			...
			Twen:Create(ImageButton,TweenInfo.new(0.85,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Position = UDim2.new(1, -60, 0.01, 0), -- ขยับปุ่มย่อหลบปุ่มปิด
			...
			Twen:Create(CloseBtn,TweenInfo1,{
				ImageTransparency = 0.3,
				Position = UDim2.new(1, -15, 0.01, 0)
			}):Play()
			...
			Twen:Create(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint),{Position = WindowTable.SavedIconPosition, BackgroundTransparency = 1, Size = UDim2.new(0, 50, 0, 50)}):Play();
			...
			Twen:Create(TabButtonFrame,TweenInfo1,{Position = UDim2.new(0.33, 0, 1.1, 0)}):Play();
			Twen:Create(MainTabFrame,TweenInfo1,{Position = UDim2.fromScale(1.5,0.23)}):Play();
			...
			Twen:Create(UICorner,TweenInfo.new(1),{
				CornerRadius = UDim.new(1,0) -- โค้ง 100% จนเป็นวงกลม
			}):Play()
	...
	MainAspectRatio.AspectRatio = 1.444 -- 650/450
	...
	Headers.Position = UDim2.new(0.005, 0, 0.01, 0)
	Headers.Size = UDim2.new(0.31, 0, 0.22, 0) -- [DeepSeek] ขยายจาก 0.195 → 0.22 ให้โลโก้หายใจได้
	Headers.ZIndex = 3
	...
	Logo.Size = UDim2.new(1.65, 0, 1.65, 0) -- ซูมให้ใหญ่ขึ้น
	Logo.ZIndex = 4
	...
	Title.Position = UDim2.new(0.33, 0, 0, 12) -- [DeepSeek] เพิ่มระยะห่างจากขอบบน 8→12
	Title.Size = UDim2.new(0.65, 0, 0.051, 0)
	Title.Font = Enum.Font.GothamBold
	Title.Text = config.Title
	Title.TextColor3 = Color3.fromRGB(255, 255, 255)
	Title.TextScaled = false
	Title.TextSize = 22.000 -- [DeepSeek] ใหญ่ขึ้นจาก 20→22 ดูโดดเด่นขึ้น
	...
	Description.Position = UDim2.new(0.33, 0, 0, 34) -- [DeepSeek] ห่างจาก Title มากขึ้น 26→34
	Description.Size = UDim2.new(0.65, 0, 0.032, 0) -- [DeepSeek] ขยายเล็กน้อยป้องกันตัดบรรทัด
	Description.Font = Enum.Font.GothamBold
	Description.Text = config.Description
	Description.TextColor3 = Color3.fromRGB(255, 255, 255)
	Description.TextScaled = false
	Description.TextSize = 15 -- [DeepSeek] ใหญ่ขึ้นเล็กน้อย 14→15 อ่านง่าย
	Description.TextTransparency = 1
	Description.TextWrapped = true
	Description.TextXAlignment = Enum.TextXAlignment.Left
	Twen:Create(Description,TweenInfo2,{TextTransparency = 0.100}):Play();
	...
	BlockFrame2.Position = UDim2.new(0.5, 0, 0.295, 0)
	...
	TabButtonFrame.Position = UDim2.new(0.33, 0, 0, 65) -- [DeepSeek] ยกขึ้นจาก 56→65 เพื่อบาลานซ์บน-ล่าง
	TabButtonFrame.Size = UDim2.new(0.65, 0, 0, 34) -- [DeepSeek] สูงขึ้นเล็กน้อย 32→34 ให้ปุ่มหายใจ
		TabButtonFrame.AnchorPoint = Vector2.new(0, 0)
	Twen:Create(TabButtonFrame,TweenInfo2,{BackgroundTransparency = 0.5}):Play();

	UICorner_6.CornerRadius = UDim.new(0, 4) -- [DeepSeek] มนขึ้นเล็กน้อย 3→4
	...
	TabButtons.ScrollBarThickness = 3 -- [DeepSeek] แถบเลื่อนบางๆ 0→3
	TabButtons.ScrollBarImageColor3 = Color3.fromRGB(120, 120, 120)
	TabButtons.ScrollBarImageTransparency = 0.5
	TabButtons.ScrollingDirection = Enum.ScrollingDirection.X

	-- [DeepSeek] UIPadding: ป้องกันปุ่ม Tab ชิดขอบเฟรม
	...
	UIListLayout.Padding = UDim.new(0, 8) -- [DeepSeek] ระยะห่างระหว่างปุ่ม Tab ให้ดูหายใจ 5→8
	...
	MainTabFrame.Position = UDim2.fromScale(0.5, 0.27) -- [DeepSeek] ยกขึ้นจาก 0.23→0.27 เพื่อลดช่องว่างใต้แถบ Tab
	MainTabFrame.Size = UDim2.new(0.96, 0, 1, -113) -- [DeepSeek] ปรับความสูงให้สัมพันธ์กัน
	Twen:Create(MainTabFrame,TweenInfo2,{BackgroundTransparency = 0.5}):Play();

	UICorner_7.CornerRadius = UDim.new(0, 5) -- [DeepSeek] มนขึ้น 3→5
	...
	function WindowTable:NewTab(cfg)
		...
		TabButton.Size = UDim2.new(0, 130, 0.9, 0)
		TabButton.ZIndex = 5
		...
		LeftFrame.ScrollBarThickness = 4 -- [DeepSeek] เพิ่ม scrollbar บางๆ 0→4
		LeftFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
		LeftFrame.ScrollBarImageTransparency = 0.4
		LeftFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y -- [DeepSeek] ขยายตามเนื้อหาอัตโนมัติ
		LeftFrame.CanvasSize = UDim2.new(0, 0, 0, 0) -- [DeepSeek] รีเซ็ต Canvas เพื่อให้ Auto ทำงานได้เต็มที่
		UIListLayout.Parent = LeftFrame
		UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
		UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
		UIListLayout.Padding = UDim.new(0, 5) -- [DeepSeek] ระยะห่างระหว่าง Section 3→5

		-- [DeepSeek] UIPadding: ป้องกันเนื้อหาชิดขอบเฟรม
		local LeftPadding = Instance.new("UIPadding")
		LeftPadding.Parent = LeftFrame
		LeftPadding.PaddingLeft = UDim.new(0, 3)
		LeftPadding.PaddingRight = UDim.new(0, 3)
		LeftPadding.PaddingTop = UDim.new(0, 3)
		...
		RightFrame.ScrollBarThickness = 4 -- [DeepSeek] เพิ่ม scrollbar บางๆ 0→4
		RightFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
		RightFrame.ScrollBarImageTransparency = 0.4
		RightFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y -- [DeepSeek] ขยายตามเนื้อหาอัตโนมัติ
		RightFrame.CanvasSize = UDim2.new(0, 0, 0, 0) -- [DeepSeek] รีเซ็ต Canvas เพื่อให้ Auto ทำงานได้เต็มที่
		UIListLayout_2.Parent = RightFrame
		UIListLayout_2.HorizontalAlignment = Enum.HorizontalAlignment.Center
		UIListLayout_2.SortOrder = Enum.SortOrder.LayoutOrder
		UIListLayout_2.Padding = UDim.new(0, 5) -- [DeepSeek] ระยะห่างระหว่าง Section 3→5
		...
		RightPadding.PaddingTop = UDim.new(0, 3)
		...
		function TabTable:NewSection(c_o_n_f_i_g)
			...
			function SectionTable:NewLabel(conf)
				...
				TextInt.BackgroundTransparency = 1.000
				TextInt.Position = UDim2.new(0.5, 0, 0.5, 0)
				TextInt.Size = UDim2.new(0.949999988, 0, 0.9, 0)
				TextInt.Font = Enum.Font.GothamBold
